...
if [ -e $PROJECT/target-ja ]; then
    docker run --rm -i -v $PROJECT:/src alpine:3.11 rm -rf /src/target-ja
fi

# Structure
docker run --rm -i \
    -v $PROJECT:/src \
    -v $PROJECT/target-ja:/src/target \
    -v $PROJECT/target-ja:/target \
    difi/vefa-structure:0.7


# Validator
docker run --rm -i -v $PROJECT:/src -v $PROJECT/target-ja:/src/target anskaffelser/validator:2.1.0 build -x -t -n eu.peppol.postaward.v3.billing -a rules,guide -target target/validator /src
...
# PINT-UBL
docker run --rm -i -v $PROJECT:/src -v $PROJECT/target-ja/generated:/target --entrypoint java klakegg/saxon:9.8.0-7 -cp /saxon.jar net.sf.saxon.Query -s:/src/rules/sch/PINT-UBL-validation-preprocessed.sch -q:tools/xquery/rules_asciidoc_peppol.xquery -o:/target/PINT-UBL-GENERAL.sch.adoc

# Guides
docker run --rm -i -v $PROJECT/guide-ja:/documents/guide -v $PROJECT/rules:/documents/rules -v $PROJECT/.postasciidoctor.sh:/documents/.postasciidoctor.sh -v $PROJECT/target-ja:/target difi/asciidoctor


# Fix ownership
docker run --rm -i -v $PROJECT:/src alpine:3.11 chown -R $(id -g $USER).$(id -g $USER) /src/target-ja
